...
(() => {
  ...
  function buildDocument(source, snapshot){
    ...
    doc.querySelectorAll('script[src],link[href]').forEach(node=>{
      ...
      if(url.origin!==location.origin || url.pathname.endsWith('/config.js') || url.pathname.endsWith('/sandbox-host-4.13.0.js')){node.remove();return;}
    ...
    runtime.src=new URL('sandbox-runtime-4.13.0.js',base).href;
