...
(() => {
  ...
  function buildDocument(source, snapshot){
    ...
    doc.querySelectorAll('script[src],link[href]').forEach(node=>{
      ...
      if(url.origin!==location.origin || url.pathname.endsWith('/config.js') || url.pathname.endsWith('/sandbox-host-4.12.9.js')){node.remove();return;}
    ...
    runtime.src=new URL('sandbox-runtime-4.12.9.js',base).href;
