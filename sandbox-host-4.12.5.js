...
(() => {
  ...
  function closeSandbox(event){
    ...
    opening++; ready=false;
    el('sandboxFrame').removeAttribute('srcdoc');
  ...
  function buildDocument(source, snapshot){
    ...
    doc.querySelectorAll('script[src],link[href]').forEach(node=>{
      ...
      if(url.origin!==location.origin || url.pathname.endsWith('/config.js') || url.pathname.endsWith('/sandbox-host-4.12.5.js')){node.remove();return;}
    ...
    runtime.src=new URL('sandbox-runtime-4.12.5.js',base).href;
  ...
  async function openSandbox(event){
    ...
    try{
      const [source,snapshot]=await Promise.all([
        fetch(new URL('index.html',location.href)).then(r=>{if(!r.ok)throw new Error('Could not load the app page');return r.text()}),
        capture()
      ]);
