...
function renderAttendancePlayerRow(p){
  ...
  const controls = canManageGames()
    ? `<div class="toggle-wrap">
        <button class="${injuryButtonClass(p)}" onclick="event.stopPropagation(); setInjuryPrompt('${p.id}')">${injuryButtonLabel(p)}</button>
        ${p.temporary ? `<button class="btn-secondary" onclick="event.stopPropagation(); makeTemporaryPlayerPermanent('${p.id}')">Make Permanent</button><button class="btn-danger" onclick="event.stopPropagation(); removePlayer('${p.id}')">Remove</button>` : ""}
      </div>`
    : "";
