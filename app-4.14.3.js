...
function bindPlayerHoldActions4132(row, p){
  ...
  row.addEventListener("pointerdown", event => {
    ...
    startScrollY = Number(window.scrollY || 0);
    row.classList.add("player-hold-arming4132");
    timer = setTimeout(() => {
      if(!tracking) return;
      // Any page movement means this was a scroll/tap sequence, not an intentional hold.
      if(Math.abs(Number(window.scrollY || 0) - startScrollY) > 2){ clearHold(); return; }
      tracking = false;
      timer = null;
      row.classList.remove("player-hold-arming4132");
      row.classList.add("player-hold-fired4132");
      row.__suppressAttendanceClickUntil4132 = Date.now() + 900;
      try{ navigator.vibrate?.(12); }catch(e){}
      openPlayerActions4132(p.id);
      setTimeout(() => row.classList.remove("player-hold-fired4132"), 180);
    }, PLAYER_HOLD_MS_4132);
  }, { passive:true });
  ...
  row.addEventListener("pointermove", event => {
    ...
    if(Math.hypot(dx, dy) > PLAYER_HOLD_MOVE_PX_4132) clearHold();
  }, { passive:true });

  ["pointerup","pointercancel","pointerleave"].forEach(type => {
    row.addEventListener(type, clearHold, { passive:true });
  });

  row.addEventListener("touchmove", clearHold, { passive:true });

  row.addEventListener("click", event => {
    if(Number(row.__suppressAttendanceClickUntil4132 || 0) > Date.now()){
...
function renderAttendancePlayerRow4132(p){
  ...
  // 4.14.3: keep Attendance cards clean. One-time actions are hold-only.
...
/* ===== 4.14.3 seamless optimistic attendance ===== */
