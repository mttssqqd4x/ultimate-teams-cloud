...
const APP_VERSION = window.ULTIMATE_TEAMS_VERSION;
...
function renderAttendancePlayerRow(p){
  ...
  const controls = canManageGames()
    ? `<div class="toggle-wrap">
        <button class="${injuryButtonClass(p)}" onclick="event.stopPropagation(); setInjuryPrompt('${p.id}')">${injuryButtonLabel(p)}</button>
        ${p.temporary ? `<button class="btn-secondary" type="button" aria-label="One-time player actions" onclick="event.stopPropagation(); openOneTimePlayerActions('${p.id}')">One-Time ▾</button>` : ""}
      </div>`
    : "";
...
/* ===== 4.13.0 one-time player conversion ===== */
async function makeTemporaryPlayerPermanent(id){
  if(!canManageGames()){
    alert("Captain or Admin only.");
    return;
  }
  ...
  if(!p.temporary){
    alert(`${p.fullName} is already a permanent player.`);
    return;
  }

  const ok = confirm(`Make ${p.fullName} a permanent player? Their current ratings and game history will be kept.`);
  if(!ok) return;

  await withLoading("Making player permanent...", async () => {
    const { error } = await db.rpc("make_temporary_player_permanent_from_app", {
      p_player_id: id
    });
    if(error) throw error;
    ...
    const msg = String(e?.message || e);
    if(msg.includes("make_temporary_player_permanent_from_app") || msg.includes("Could not find the function")){
      alert("The permanent-player database function is missing. Run update_4_13_0.sql in Supabase, then try again.");
    }else{
      alert("Could not make this player permanent: " + msg);
    }
  });
}

Object.assign(window, { makeTemporaryPlayerPermanent });


/* ===== 4.13.1 combined one-time player actions ===== */
function closeOneTimePlayerActions(){
  document.getElementById("oneTimePlayerActionsModal")?.remove();
}

function openOneTimePlayerActions(id){
  if(!canManageGames()){
    alert("Captain or Admin only.");
    return;
  }
  ...
  closeOneTimePlayerActions();

  const wrap = document.createElement("div");
  wrap.id = "oneTimePlayerActionsModal";
  wrap.className = "modal-backdrop modal-open";
  wrap.innerHTML = `
    <div class="modal-card" style="width:min(420px,100%);max-height:none">
      <div class="row" style="justify-content:space-between;align-items:center;gap:10px;margin-bottom:12px">
        <h2 style="margin:0;font-size:18px">${escapeHtml(p.fullName)}</h2>
        <button class="btn-secondary" type="button" id="oneTimeActionsClose" style="width:auto">Close</button>
      </div>
      <div class="grid grid-1" style="gap:9px">
        <button class="btn" type="button" id="oneTimeMakePermanent">Make Permanent</button>
        <button class="btn-danger" type="button" id="oneTimeRemove">Remove</button>
      </div>
    </div>
  `;

  wrap.querySelector("#oneTimeActionsClose").onclick = closeOneTimePlayerActions;
  wrap.querySelector("#oneTimeMakePermanent").onclick = () => {
    closeOneTimePlayerActions();
    makeTemporaryPlayerPermanent(id);
  };
  wrap.querySelector("#oneTimeRemove").onclick = () => {
    closeOneTimePlayerActions();
    removePlayer(id);
  };
  wrap.addEventListener("click", event => {
    if(event.target === wrap) closeOneTimePlayerActions();
  });

  document.body.appendChild(wrap);
}

Object.assign(window, { openOneTimePlayerActions, closeOneTimePlayerActions });
