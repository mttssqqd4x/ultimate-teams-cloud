...
// One gesture per document. Row-local timers survive DOM replacement and can
// mistake a later tap for an earlier hold. End gestures at window capture phase.
let attendanceGesture4144 = null;
let attendanceHoldClick4144 = null;
...
function scheduleAttendanceRender4144(){
  ...
  attendanceRenderTimer4144 = setTimeout(() => {
    ...
    renderPlayers();
  }, 160); // Let the release's native click finish before replacing its target.
}

function cancelAttendanceGesture4144(){
  const gesture = attendanceGesture4144;
  attendanceGesture4144 = null;
  if(gesture){
    attendanceRenderAfter4144 = Date.now() + 160;
    clearTimeout(gesture.timer);
    gesture.row.classList.remove("player-hold-arming4132");
  }
...
window.addEventListener("pointerdown", () => {
  ...
  attendanceHoldClick4144 = null;
}, { capture:true, passive:true });
window.addEventListener("pointerup", event => {
  if(attendanceGesture4144?.pointerId === event.pointerId) cancelAttendanceGesture4144();
}, { capture:true, passive:true });
window.addEventListener("pointercancel", cancelAttendanceGesture4144, { capture:true, passive:true });
window.addEventListener("pointermove", event => {
  const g = attendanceGesture4144;
  if(!g || event.pointerId !== g.pointerId) return;
  if(Math.hypot(event.clientX - g.x, event.clientY - g.y) > PLAYER_HOLD_MOVE_PX_4132)
    cancelAttendanceGesture4144();
}, { capture:true, passive:true });
// Native touch termination is a fallback for interrupted WebKit pointer streams.
window.addEventListener("touchend", cancelAttendanceGesture4144, { capture:true, passive:true });
window.addEventListener("touchcancel", cancelAttendanceGesture4144, { capture:true, passive:true });
window.addEventListener("scroll", cancelAttendanceGesture4144, { capture:true, passive:true });
window.addEventListener("blur", cancelAttendanceGesture4144);
window.addEventListener("pagehide", cancelAttendanceGesture4144);
document.addEventListener("visibilitychange", () => {
  if(document.hidden) cancelAttendanceGesture4144();
});
window.addEventListener("click", event => {
  if(!event.target.closest?.("#playerList .player")) return;
  cancelAttendanceGesture4144();
  const heldRow = attendanceHoldClick4144;
  attendanceHoldClick4144 = null;
  if(heldRow?.contains(event.target)){
...
function bindPlayerHoldActions4132(row, p){
  if(!row || !p || !canManageGames() || row.dataset.holdBound4132 === "true") return;
  row.dataset.holdBound4132 = "true";
  ...
  row.addEventListener("pointerdown", event => {
    ...
    const g = { row, pointerId:event.pointerId, x:event.clientX, y:event.clientY, timer:null };
    attendanceGesture4144 = g;
    row.classList.add("player-hold-arming4132");
    g.timer = setTimeout(() => {
      ...
      // Only this uninterrupted contact can consume its following click.
      attendanceHoldClick4144 = row;
      row.classList.remove("player-hold-arming4132");
      try{ navigator.vibrate?.(12); }catch(e){}
      openPlayerActions4132(p.id);
    }, PLAYER_HOLD_MS_4132);
  }, { passive:true });
...
function renderAttendancePlayerRow4132(p){
  ...
  // 4.14.4: keep Attendance cards clean. One-time actions are hold-only.
...
/* ===== 4.14.4 seamless optimistic attendance ===== */
...
function applyPendingAttendance4142(){
  for(const [id, pending] of [...attendancePending4142.entries()]){
    const p = attendancePlayer4142(id);
    if(!p) continue;
    const serverValue = !!p.attending;
    if(pending.confirmed && serverValue === !!pending.desired){
...
const loadCloudDataBefore4142 = loadCloudData4120;
loadCloudData4120 = async function(options = {}){
  const result = await loadCloudDataBefore4142(options);
...
async function syncAttendancePlayer4142(playerId){
  ...
  try{
      ...
      try{
        ...
        if(error){
          if(isNetworkError4120?.(error)){
            queueAttendance4120(playerId, desired);
            break;
          }
          throw error;
        }
        ...
        if(latest && latest.seq === seq){
          ...
          // A realtime event normally confirms this. The small fallback refresh
          // also handles browsers that temporarily suspend realtime callbacks.
          setTimeout(async()=>{
            try{
              await loadCloudData4120({ includeProfile:true, applyLocal:true, force:true });
              renderAll();
    ...
    if(latest && !latest.confirmed && navigator.onLine !== false){
      ...
      queueMicrotask(()=>syncAttendancePlayer4142(playerId));
    }
  }
}
