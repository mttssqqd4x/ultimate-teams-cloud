...
- 4.14.3: serializes rapid attendance writes per player so older requests cannot overwrite a newer tap.
- 4.14.3: fixes the iOS Home Screen/standalone top blur by using a solid safe-area cap and disabling header backdrop blur only in installed-app mode.

- 4.14.3: removes direct One-Time controls from Attendance and hides the 100% injury control; injured players keep a visible read-only injury indicator while all editing/actions are accessed by press-and-hold.
...
## 4.9.10 Mobile popup horizontal-drift fix

Changes:
- locked modals and popup lists to vertical scrolling on mobile
- hid horizontal overflow inside popup cards and lists
